...
<spring:message code="mmc.add.operacao" var="mccAddOperacao"/>
<spring:message code="mmc.del.operacao" var="mccDelOperacao"/>

	<header class="bg-primary text-white">
		<div class="container text-center">
			<h1>
				<fmt:message key="mmc.title" />
			</h1>
			<p class="lead">
				<fmt:message key="mmc.description" />
			</p>
		</div>
	</header>
		...
		<div class="container">
			...
			<form action="${s:mvcUrl('calcular_mmc').build() }" method="post">
				<div class="form-group">
					...
					<div>
						<p>
							<input class="btn btn-primary" type="button" value="${mccAddOperacao}" onclick="add();"></input> 
							<input class="btn btn-primary" type="button" value="${mccDelOperacao}" onclick="remove();"/>
						</p>
					</div>
				</div>
				<button type="submit" class="btn btn-primary">
					<fmt:message key="mmc.calculate" />
				</button>
			</form>
